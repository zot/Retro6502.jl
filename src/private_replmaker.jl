...
function initrepl(
    ...
    lang_keymap = Dict{Any,Any}(
        normalized_start_key =>
            (s, args...) -> if isempty(s) || position(LineEdit.buffer(s)) == 0
                enter_mode!(s, lang_mode, enter, entered)
            else
                alt(s, args...)
            end,
    )

    lang_mode.keymap_dict = LineEdit.keymap(
        Dict{Any,Any}[
            skeymap,
            mk,
            prefix_keymap,
    ...
    startup_text && println(
        "REPL mode $mode_name initialized. Press $start_key to enter and backspace to exit.",
    )
...
function get_nested_key(keymap::Dict, key::Union{String,Char})
    ...
    while y !== nothing
        ...
        if (y === nothing) == isa(tmp, Dict)
            error(
                "Conflicting definitions for keyseq " *
                escape_string(key) *
                " within one keymap",
            )
        end
...
function enter_mode!(s::LineEdit.MIState, lang_mode, enter, entered)
...
function enter_mode!(lang_mode)
    enter_mode!(Base.active_repl.mistate, lang_mode, () -> nothing, () -> nothing)
end
